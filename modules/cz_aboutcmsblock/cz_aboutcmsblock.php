...
class Cz_Aboutcmsblock extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->trans('CZ - About CMS Block', array(), 'Modules.AboutCmsBlock');
    ...
    public function installDB()
    {
        ...
        $return &= Db::getInstance()->execute('
                ...
                `text` text NOT NULL,
                PRIMARY KEY (`id_czaboutcmsblockinfo`, `id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8 ;'
        );
    ...
    public function uninstallDB($drop_table = true)
    {
        $ret = true;
        if ($drop_table) {
            $ret &=  Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'czaboutcmsblockinfo`') && Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'czaboutcmsblockinfo_lang`');
        }
    ...
    public function processSaveAboutCmsBlock()
    {
        ...
        if (Shop::isFeatureActive() && !$czaboutcmsblockinfo->id_shop) {
            $saved = true;
            $shop_ids = Shop::getShops();
            foreach ($shop_ids as $id_shop) {
                $czaboutcmsblockinfo->id_shop = $id_shop;
                $saved &= $czaboutcmsblockinfo->add();
            }
        } else {
			$czaboutcmsblockinfo->id_shop = Shop::getContextShopID();
    ...
    {
        $fields_value = array();
        $id_czaboutcmsblockinfo = 1;

        foreach (Language::getLanguages(false) as $lang) {
            $czaboutcmsblockinfo = new AboutCmsBlock((int)$id_czaboutcmsblockinfo);
            $fields_value['text'][(int)$lang['id_lang']] = $czaboutcmsblockinfo->text[(int)$lang['id_lang']];
        }

        $fields_value['id_czaboutcmsblockinfo'] = $id_czaboutcmsblockinfo;
    ...
    {
        $sql = 'SELECT r.`id_czaboutcmsblockinfo`, r.`id_shop`, rl.`text`
            FROM `'._DB_PREFIX_.'czaboutcmsblockinfo` r
            LEFT JOIN `'._DB_PREFIX_.'czaboutcmsblockinfo_lang` rl ON (r.`id_czaboutcmsblockinfo` = rl.`id_czaboutcmsblockinfo`)
