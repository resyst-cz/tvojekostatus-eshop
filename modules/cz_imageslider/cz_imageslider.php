...
include_once(_PS_MODULE_DIR_.'cz_imageslider/Cz_HomeSlide.php');

class Cz_ImageSlider extends Module implements WidgetInterface
{
    protected $_html = '';
    protected $default_speed = 8000;
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()->trans('CZ - Image slider', array(), 'Modules.ImageSlider');
        $this->description = $this->getTranslator()->trans('Display Image Slider on Homepage.', array(), 'Modules.ImageSlider');
        $this->ps_versions_compliancy = array('min' => '1.7.0.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:cz_imageslider/views/templates/hook/slider.tpl';
    }
    ...
    public function install()
    {
        ...
        if (parent::install() &&
            ...
            foreach ($shops as $shop_id) {
                ...
                $res = Configuration::updateValue('CZHOMESLIDER_SPEED', $this->default_speed, false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('CZHOMESLIDER_PAUSE_ON_HOVER', $this->default_pause_on_hover, false, $shop_group_id, $shop_id);
            }

            /* Sets up Shop Group configuration */
            if (count($shop_groups_list)) {
                foreach ($shop_groups_list as $shop_group_id) {
                    $res &= Configuration::updateValue('CZHOMESLIDER_SPEED', $this->default_speed, false, $shop_group_id);
                    $res &= Configuration::updateValue('CZHOMESLIDER_PAUSE_ON_HOVER', $this->default_pause_on_hover, false, $shop_group_id);
                }
            }

            /* Sets up Global configuration */
            $res &= Configuration::updateValue('CZHOMESLIDER_SPEED', $this->default_speed);
            $res &= Configuration::updateValue('CZHOMESLIDER_PAUSE_ON_HOVER', $this->default_pause_on_hover);
    ...
    public function uninstall()
    {
        ...
        if (parent::uninstall()) {
            ...
            $res &= Configuration::deleteByName('CZHOMESLIDER_SPEED');
            $res &= Configuration::deleteByName('CZHOMESLIDER_PAUSE_ON_HOVER');
    ...
    protected function createTables()
    {
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'czhomeslider_slides` (
              `id_czhomeslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'czhomeslider_slides_lang` (
              `id_czhomeslider_slides` int(10) unsigned NOT NULL,
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new Cz_HomeSlide($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'czhomeslider`, `'._DB_PREFIX_.'czhomeslider_slides`, `'._DB_PREFIX_.'czhomeslider_slides_lang`;
        ');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = Cz_HomeSlide::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function _postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            if (!Validate::isInt(Tools::getValue('CZHOMESLIDER_SPEED'))) {
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            foreach ($shops as $shop_id) {
                ...
                $res = Configuration::updateValue('CZHOMESLIDER_SPEED', (int)Tools::getValue('CZHOMESLIDER_SPEED'), false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('CZHOMESLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('CZHOMESLIDER_PAUSE_ON_HOVER'), false, $shop_group_id, $shop_id);
            }

            /* Update global shop context if needed*/
            switch ($shop_context) {
                case Shop::CONTEXT_ALL:
                    $res &= Configuration::updateValue('CZHOMESLIDER_SPEED', (int)Tools::getValue('CZHOMESLIDER_SPEED'));
                    $res &= Configuration::updateValue('CZHOMESLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('CZHOMESLIDER_PAUSE_ON_HOVER'));
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue('CZHOMESLIDER_SPEED', (int)Tools::getValue('CZHOMESLIDER_SPEED'), false, $shop_group_id);
                            $res &= Configuration::updateValue('CZHOMESLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('CZHOMESLIDER_PAUSE_ON_HOVER'), false, $shop_group_id);
                        }
                    }
                    break;
            ...
            $slide = new Cz_HomeSlide((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            $slide = new Cz_HomeSlide((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    {
        $this->context->controller->registerStylesheet('modules-czhomeslider', 'modules/'.$this->name.'/views/css/nivo-slider.css', ['media' => 'all', 'priority' => 150]);
        $this->context->controller->registerJavascript('modules-czhomeslider', 'modules/'.$this->name.'/views/js/jquery.nivo.slider.js', ['position' => 'bottom', 'priority' => 150]);
    }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN '._DB_PREFIX_.'czhomeslider_slides hss ON (hs.id_czhomeslider_slides = hss.id_czhomeslider_slides)
            LEFT JOIN '._DB_PREFIX_.'czhomeslider_slides_lang hssl ON (hss.id_czhomeslider_slides = hssl.id_czhomeslider_slides)
            WHERE hs.`id_czhomeslider_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = Cz_HomeSlide::getAssociatedIdsShop((int)$slide['id_slide']);
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'name' => 'CZHOMESLIDER_PAUSE_ON_HOVER',
    ...
    public function getConfigFieldsValues()
    {
        ...
        return array(
            'CZHOMESLIDER_SPEED' => Tools::getValue('CZHOMESLIDER_SPEED', Configuration::get('CZHOMESLIDER_SPEED', null, $id_shop_group, $id_shop)),
            'CZHOMESLIDER_PAUSE_ON_HOVER' => Tools::getValue('CZHOMESLIDER_PAUSE_ON_HOVER', Configuration::get('CZHOMESLIDER_PAUSE_ON_HOVER', null, $id_shop_group, $id_shop)),
        );
    }
