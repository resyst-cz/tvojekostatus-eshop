...
class Cz_HomeSlide extends ObjectModel
{
	public $title;
	public $url;
	public $legend;
	...
	public static $definition = array(
		'table' => 'czhomeslider_slides',
		'primary' => 'id_czhomeslider_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'czhomeslider` (`id_shop`, `id_czhomeslider_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'czhomeslider`
			WHERE `id_czhomeslider_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_czhomeslider_slides` as id_slide
			FROM `'._DB_PREFIX_.'czhomeslider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'czhomeslider` hs ON (hss.`id_czhomeslider_slides` = hs.`id_czhomeslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new Cz_HomeSlide($row['id_slide']);
