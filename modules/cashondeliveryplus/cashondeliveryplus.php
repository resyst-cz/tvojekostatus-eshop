...
{	private $fee;
   ...
   public   function uninstall()
    {
        if (!Configuration::deleteByName('COD_FEE') || !Configuration::deleteByName('COD_STATE')  || !Configuration::deleteByName('COD_FEEFREE') ||   !Configuration::deleteByName('COD_CARRIERS') || !parent::uninstall())
            return false;
        return true;
    }
        ...
        public function hookPaymentOptions($params)
    {
      ...
      if(!$this->isAllowedCarrier($this->getCarrierId($params))) 
        ...
        Context::getContext()->smarty->assign(array(
         ...
         $amountPaid = Context::getContext()->cart->getOrderTotal(true, Cart::BOTH_WITHOUT_SHIPPING); 
         if($this->feefree > 0 && $amountPaid >  Tools::convertPrice($this->feefree)) {
             Context::getContext()->smarty->assign(array(
            'codfee' => $this->l('Free!')));  
         } else {
        ...
        $newOption->setModuleName($this->name)
             ->setCallToActionText($this->l('Pay by Cash on Delivery'))
            ->setAction($this->context->link->getModuleLink($this->name, 'validation', array(), true))
            ->setAdditionalInformation($this->fetch('module:cashondeliveryplus/views/templates/hook/payment.tpl'));
	...
	public function hookPaymentReturn($params)
	{
		if (!$this->active)
			return ;
      if(Tools::version_compare(_PS_VERSION_, '1.7', '>')) {
         return $this->display(__FILE__, 'confirmation17.tpl'); 
      }
  ...
  /**
    ...
    */
            ...
            foreach ($package_list as $id_address => $packageByAddress)
                foreach ($packageByAddress as $id_package => $package)
                {
                    ...
                    $order->total_discounts = $order->total_discounts_tax_incl;
                      ...
                      /////////////////////////////////////////////////////////////  
                   if($this->feefree > 0 && $order->total_products_wt > Tools::convertPrice($this->feefree)) {
                        $fee=0;     
                     }
                     else {
    ...
    {   
        ...
        $this->_html.=$this->l('Fee: ').'<input  name="COD_FEE" value="'.Tools::convertPrice($this->fee).'" />   '.$currency->sign.'  '.$this->l('Including tax if any').' <br />
        '.$this->l('Fee free from:  ').'<input  name="COD_FEEFREE" value="'.Tools::convertPrice($this->feefree).'" />   '.$currency->sign.'   <br />' 
        .$this->l('0 = always pay a fee').'    <br />';
        
        
            $this->_html .='<br /><br />'.$this->l('Allowed carriers').'<br />'.$this->fetchCarriers($this->allowed_carriers).'<br /> 
  

          
        <input class="button" name="btnSubmit" value="'.$this->l('Save').'" type="submit" /> 
            </fieldset>';
    ...
    {
        $this->_html .= '<img src="../modules/cashondeliveryplus/cashondeliveryplus.jpg" style="float:left; margin-right:15px;"><b>'.$this->l('Module Cash on Delivery').'<br /></b><br />';
       ...
       <b><a href="http://www.prestahost.cz" target="_blank">Prestahost.cz</a> </b>: česká podpora Prestashopu, specializovaný hosting, moduly na míru, import xml feedů ...
        ...
        <br /> 
         Tento modul byl upraven aby  umožňil vazbu na dopravce, nastavitelné dobírečné, dobírečné zdarma od....  <br />   <br /> ';  
    } 
