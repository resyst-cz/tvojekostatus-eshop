...
class FOOTERSHOPINFO extends Module
{
	public function __construct()
	{
		$this->name = 'footershopinfo';
		$this->description = 'Informace o obchodě v patičce';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Footershopinfo_MONOLANGUAGE';
		$this->text_editor_content = 'Footershopinfo_TEXTEDITOR';
		$this->content_wrapper = 'Footershopinfo_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Footershopinfo_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Footershopinfo_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('Footer shop info');
		$this->description = $this->l('Informace o obchodě v patičce');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !FOOTERSHOPINFOModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !FOOTERSHOPINFOModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = FOOTERSHOPINFOModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = FOOTERSHOPINFOModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					FOOTERSHOPINFOModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					FOOTERSHOPINFOModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																FOOTERSHOPINFOModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class FOOTERSHOPINFOModel extends ObjectModel
{

	public static $definition = array(
		'table' => 'footershopinfo',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( FOOTERSHOPINFOModel::createFilesTable()
				&& FOOTERSHOPINFOModel::createContentTable());
	}
