...
class LeftBanner extends ObjectModel
{
	...
	public static $definition = array(
		'table' => 'czleftbanner_slides',
		'primary' => 'id_czleftbanner_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'czleftbanner` (`id_shop`, `id_czleftbanner_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'czleftbanner`
			WHERE `id_czleftbanner_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_czleftbanner_slides` = hs.`id_czleftbanner_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_czleftbanner_slides` as id_slide
			FROM `'._DB_PREFIX_.'czleftbanner_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'czleftbanner` hs ON (hss.`id_czleftbanner_slides` = hs.`id_czleftbanner_slides`)
